...
static u8  slave_ctrl_ID      =  SF_SlaveID_1;
...
//			RS485LedToggle;										//有线通信灯翻转
            ...
            if(pBuf[1]==0x10)
            {
                ...
                if(pBuf[3]>=0x28&&pBuf[3]<0x44)   //PID设定参数保存,由网关下发，寄存器地址+40，从40到68
                {
                 ...
                 if(pBuf[3] == 0x64 && pBuf[5] == 8)   //网关采样配肥流量下发,三路流量和总管流量，往站地址为SF_SlaveID_0与SF_SlaveID_1的两个控制器发送
                {
                    SF_flg =1;
...
void ctrRoller1_1(void)                //子站36  第一路流量电机控制
{
    ...
    if(SF_Set_Flow[0] != 0  && SF_Flow_Total > 10)  //总管无流量 或设定值为0时直接 关阀门
    {
        ...
        if(error >(( SF_Set_Flow[0] * HIGHERR )) )                    //偏差大于设定值得90%，给300ms的脉冲，三下打开阀门
        {
            T_ON_1 = 100;         //给1个100ms的脉冲 快速打开阀门
        }
        else if(error >(SF_Set_Flow[0] * MIDERR))     //如果设定流量与实测流量相差超过20%，电机快速转动
        {
            T_ON_1 = HIGHSPEED;
        }
        else if((error <=(SF_Set_Flow[0] * MIDERR))&&(error >(SF_Set_Flow[0] * LOWERR)))     //误差在0.2-0.1之间 给25ms的脉冲 微调
        {
            T_ON_1 = MIDSPEED;
        }
        else if((error <=(SF_Set_Flow[0] * LOWERR))&&(error >(SF_Set_Flow[0] * DEADERR)))     //误差在0.1-0.3之间 给18ms脉冲 轻微调
        {
            T_ON_1 = LOWSPEED;
        }
        else if(error <= SF_Set_Flow[0] * DEADERR)     //误差在10%  死区
        {
            T_ON_1 = HOLDSPEED;
        }
        if(motor1_flg == 1)
...
void ctrRoller1_2(void)                //子站36 第二路流量电机控制
{
    ...
    if(SF_Set_Flow[1] != 0  && SF_Flow_Total > 10)
    {
        ...
        if(error >(( SF_Set_Flow[1] * HIGHERR )) )                    //偏差大于设定值得90%，给100ms的脉冲，快速开阀门并且调整流量，-40是因为零点偏移
        {
            T_ON_2 = 100;         //给1个100ms的脉冲 快速打开阀门
        }
        else if(error >(SF_Set_Flow[1] * MIDERR))     //如果设定流量与实测流量相差超过50%，电机快速转动
        {
            T_ON_2 = HIGHSPEED;
        }
        else if((error <=(SF_Set_Flow[1] * MIDERR))&&(error >(SF_Set_Flow[1] * LOWERR)))     //误差在0.3-0.5之间  微调
        {
            T_ON_2 = MIDSPEED;
        }
        else if((error <=(SF_Set_Flow[1] * LOWERR ))&&(error >(SF_Set_Flow[1]* DEADERR )))     //误差在0.1-0.3之间 给轻微调
        {
            T_ON_2 = LOWSPEED;
        }
        else if(error <= SF_Set_Flow[1] * DEADERR)     //误差在10%  死区，阀门保持不动
        {
            T_ON_2 = HOLDSPEED;   
        }
        if(motor2_flg == 1)
...
void ctrRoller2_1(void)                //子站37  第三路流量电机控制
{
    ...
    if(SF_Set_Flow[2] != 0 && SF_Flow_Total > 10)
    {
        ...
        if(error >(( SF_Set_Flow[2] * HIGHERR )) )                    //偏差大于设定值得90%，给100ms的脉冲
        {
            T_ON_3 = 100;         //给1个100ms的脉冲 快速打开阀门
        }
        else if(error >(SF_Set_Flow[2] * MIDERR))     //如果设定流量与实测流量相差超过50%，电机快速转动
        {
            T_ON_3 = HIGHSPEED;
        }
        else if((error <=(SF_Set_Flow[2] * MIDERR))&&(error >(SF_Set_Flow[2] *LOWERR)))     //误差在0.2-0.1之间 微调
        {
            T_ON_3 = MIDSPEED;
        }
        else if((error <=(SF_Set_Flow[2] * LOWERR))&&(error >(SF_Set_Flow[2] * DEADERR)))     //误差在0.1-0.05之间  轻微调
        {
            T_ON_3 = LOWSPEED;
        }
        else if(error <= SF_Set_Flow[2] *DEADERR)     //误差在5%  死区
        {
            T_ON_3 = HOLDSPEED;
        }
        if(motor3_flg == 1)
