...
/**
  ...
  * @retval None
  */
void USART_SendData(USART_TypeDef* USARTx, uint16_t Data) //uint16_t
{
	u32 i;
	i=0x004FFFFF;//防止全双工硬件通信时，硬件CTS不起作用造成等待状态反复进行watchdog(3S)不停起作用;2S左右，以看门狗不起作用为准
  /* Check the parameters */
  assert_param(IS_USART_ALL_PERIPH(USARTx));
  ...
  while((USART_GetFlagStatus(USARTx, USART_FLAG_TXE) == RESET)&&(i!=0)){i--;} //&&(i!=0)
}
