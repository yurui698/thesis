...
__attribute__((section("NO_INIT"),zero_init))  u8 Collectors[65][16];//光照度、空气湿度、空气温度、土壤水分（液位）、土壤温度（电机电流）、二氧化碳；其余保留
...
u8 bytelen3=0;//Send_slave_cmd函数的临时变量定义

static void WriteDataToBuffer(u8 port,u8 *ptr,u8 start,u8 len);

u8 WriteMultipleRegister(u8 Slave_ID,u16 addr,u16 num,u8 *pData,u8 *temp);
...
/*采集方式，参数个数，滤波次数；采集方式：00 无采集，01 光照 k=100 b=0，02 空气温湿度 湿度k=524.29 b=-6 温度 k=372.96 b=-46.85，
	...
	以下为控制器设定
         32 通知控制器为控制采集方式（水肥机施灌区控制阀及RS485采集使用），通道4方式（第5通道），通道5与采集器相同。
...
u8  factory_gateway_set[255]=            //触摸屏下发出厂网关设定参数需要写入flash0x0805 C000，从256k开始写入，0x0804加1，则增加64k,每次写入必需2Kbyte
{
    ...
    34,07,01,	//网关采集通道5参数设定;factory_gateway_set[27]~[29]
...
__attribute__((section("NO_INIT"),zero_init))  u8  wgcollector_data_buff[16];
...
u8 TDlvbo_number[MaxTD_number]= {50,50,50};	//通道0的50次数据求平均值，4秒/次
//通道1的50次数据求平均值，4秒/次
static  u16 Adc_average[MaxTD_number][Maxlvbo_number]; //ADC求平均值数组，存放历史采集数据
u8 tdcycle_i[MaxTD_number]= {0,0,0}; //循环次数数组，0~TDlvbo_number[i]-1，i为通道号，i=0、1、....
u8 First_adc_average[3]= {0,0,0}; //首次滤波，当First_adc_average=0时，调用First_Getaverage(u8 TD_number,u8 Maxlvbo_number,u16 temp_adc)
//否则调用  TD_Getaverage(u8 TD_number,u8 TDlvbo_number[TD_number],u16 temp_adc,u8 tdcycle_i[TD_number])
u16 collector_temp,temp_adc,temp_flow,temp_level; //传感器检测数据暂存变量
...
static u16 send_Collectors_time=509;
...
void WriteDataToDMA_BufferTX3(uint16_t size);
...
u8 set_fertigation51_finish[28];
u8 read_fertigation_finish51;
...
u8 param_fertigation52[36];
u8 fertigation_flg=0;//用于区分常用网关还是水肥网关，根据不同的触摸屏程序，在触摸屏下发查询地址0x52时自动区分（置1）。
/*用做水肥机网关，从41#~64#控制器也作为9#~32#采集器上报6个参数，最后二个参数供水肥机触摸屏使用*/
//static u8 test[100];
extern u8 SF_cmd_num[28][2];
extern u8 SF_Lvbo641;
extern u8 SF_Lvbo642;
extern   u8 SF_Lvbo646;
...
extern u16 SF_Flow_Total[6];
extern	u8 SF_Flow_Total_I;
extern u8 SF_Flow_Total_flg ;
extern	u8 SF_wgcollector_data_buff[16] ;
extern u8 fertigation52databuf[12];
...
void Period_Events_Handle(u32 Events)
{
    ...
    if(Events&SYS_INIT_EVT)
    {
        if(RCC_GetFlagStatus(RCC_FLAG_PORRST)!=RESET)
        {
            ...
            memset(init_cmd_numYX, 20, sizeof(init_cmd_numYX));
            memset(init_cmd_numWX, 10, sizeof(init_cmd_numWX));
            ZZ_temp_stateYX=0;
            ZZ_temp_stateWX=0;
        }
        RCC_ClearFlag();
...
//		Get_WX_Channel();//无线信道选择初始化
        ...
        {
            SI4463_Init();
            Start_timerEx(WX_SENDZZ_EVT,3333);
        }//触摸屏出厂设置应加入变量
//    RXENABLE3;//测试
        RXENABLE4;
        RXENABLE5;
        ...
        Start_timerEx(WG_SENDZZ_EVT,3313);
        Start_timerEx(NET_INIT_EVT,3000);
        if(factory_gateway_set[11]!=0)
        {
            u16  logic_ture;
            if((factory_gateway_set[12]>=6&&factory_gateway_set[12]<=7)||factory_gateway_set[12]==10||factory_gateway_set[12]==28||factory_gateway_set[12] == 33)//开关量及频率信号输入管脚PC0上拉20k电阻；通道0（光照）=10 cps120压力输入
            {
            ...
            logic_ture=logic_ture||factory_gateway_set[15]==23;
						logic_ture = logic_ture || factory_gateway_set[12] == 33 || factory_gateway_set[15] == 33 || factory_gateway_set[24] == 33;
...
static void RxReport3(u8 len,u8 *pData) //与子站通信，接收子站回复
{
    if (GetCRC16(pData, len) == 0)
    {
        Start_timerEx(WG_SENDZZ_EVT, 149);
