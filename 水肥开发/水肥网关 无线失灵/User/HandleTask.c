...
static u8 SF_wgcollector_data_buff[20] = {0};
...
void Period_Events_Handle(u32 Events)
{
    ...
    if (Events&SYS_INIT_EVT)
    {
        ...
        {
            SI4463_Init();
            Start_timerEx(WX_SENDZZ_EVT, 4000);
        }//触摸屏出厂设置应加入变量
//    RXENABLE3;//测试
        RXENABLE4;
        RXENABLE5;
        ...
        Start_timerEx(WG_SENDZZ_EVT, 4333);
...
void SF_Flow_Measu()   //三路配肥流量监测
{
    ...
    if(factory_gateway_set[12]==33 ||factory_gateway_set[15]==33 ||factory_gateway_set[24]==33)
    {
        ...
        SF_wgcollector_data_buff[14]=param_fertigation52[36] | (param_fertigation52[37]<<8);        //正在施肥标志 =1则表明开始施肥 =0则表明 没有施肥
				
				SF_wgcollector_data_buff[15]=param_fertigation52[30] | (param_fertigation52[31]<<8);  //第一路允许施肥
				SF_wgcollector_data_buff[16]=param_fertigation52[32] | (param_fertigation52[33]<<8);  //第二路允许施肥
				
				SF_wgcollector_data_buff[17]=param_fertigation52[34] | (param_fertigation52[35]<<8);  //第三路允许施肥
...
void SF_Flow_Trans()   //配肥三路直管瞬时流量下发，总管流量电磁流量计485,寄存器地址固定0x64
{
    ...
    case 0:
        ...
        bytelen3 = WriteMultipleRegister(SF_SlaveID_0, 100, 9, SF_wgcollector_data_buff, ReportData3); //寄存器100用于存储三路流量数据
        ...
        break;
    case 1:
        bytelen3 = WriteMultipleRegister(SF_SlaveID_1, 100,9, SF_wgcollector_data_buff, ReportData3);
        memcpy(SF_USART3SendTCB, ReportData3, bytelen3);
        SF_WriteDataToDMA_BufferTX3(bytelen3);
        SF_Flow_Trans_Flg = 0;
        break;
